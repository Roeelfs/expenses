...
description: Identifies the format of a bank/credit-card statement and wires it into the engine — writing a parser when the format isn't supported yet, and adding the matching Source entries to config.py. Use during onboarding when a user provides statements in an unknown layout. Note: the primary data source is now the Moneytor adapter (map_moneytor) — use this agent only when a user has raw statement files that Moneytor does not cover.
...
**Note:** The default data source is now Moneytor — `engine/moneytor.py` fetches
transactions and `analyze.py:map_moneytor` normalizes them through the same
`Source(kind=...)` seam. Use this agent only when the user has statement files
that Moneytor does not cover (e.g. a bank not yet aggregated by Moneytor). In
future, a `scraper` adapter behind the same `Source` seam is the intended
alternative to raw-file parsers.
...
6. **Verify**: `python3 engine/analyze.py` runs clean; the new file's rows appear
   in `output/report.html`; counts look sane. Spot-check 3–5 rows against the
   source.
